...
export async function runStatus(): Promise<void> {
  ...
  if (!fileExists(paths.state) && !fileExists(paths.handoff)) {
    console.log(pc.yellow("No .aicontext yet. Run `agentbridge init`."));
    return;
  }
  ...
  if (changed === -1) {
    out.push(section("Index", pc.yellow("no index yet — run `agentbridge index`")));
  } else {
    const msg =
      changed === 0
        ? pc.green("up to date")
        : pc.yellow(`${changed} file(s) changed since last index — run \`agentbridge index\``);
