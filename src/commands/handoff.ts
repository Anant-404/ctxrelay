...
function checkIndexFreshness(paths: ProjectPaths): void {
  if (!fileExists(paths.index)) {
    console.error(pc.yellow("note: no index.json yet — run `agentbridge index`"));
    return;
  }
  try {
    ...
    if (newest > indexMtime) {
      console.error(pc.yellow("note: source changed since last index — run `agentbridge index`"));
    }
  } catch {
    /* ignore */
  }
